...
// Check if the "imageFileName" key exists in $_FILES
if (isset($_FILES["imageFileName"])) {
    $targetFile = $targetDir . basename($_FILES["imageFileName"]["name"]);
    $uploadOk = 1;
    $imageFileType = strtolower(pathinfo($targetFile, PATHINFO_EXTENSION));

    // Check file size (limit to 5MB)
    if ($_FILES["imageFileName"]["size"] > 5000000) {
    ...
    if ($uploadOk) {
        if (move_uploaded_file($_FILES["imageFileName"]["tmp_name"], $targetFile)) {
            // Image file uploaded successfully
            echo "The file " . basename($_FILES["imageFileName"]["name"]) . " has been uploaded.";

            // Insert data into the database
            $sql = "INSERT INTO plasticscrap (supplier_id, productName, pricePerKg, color, source, thickness, packagingSize, material, minOrderQuantity, imageFileName) 
                VALUES ('$supplier_id', '$productName', '$pricePerKg', '$color', '$source', '$thickness', '$packagingSize', '$material', '$minOrderQuantity', '$targetFile')";
        ...
        // Handle the case where "imageFileName" is not set in $_FILES
...
<body class="sb-nav-fixed">
    ...
    <h2 class="mb-4">Plastic Scrap Form</h2>
    <form action="PlasticScrapForm.php" method="post" enctype="multipart/form-data">
        <div class="row">
